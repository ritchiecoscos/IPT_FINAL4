<div>
    <div class="container mt-3">
        <h1 style="font-size: 25px; font-weight: 500; color: black;">Permissions</h1>
        @include('livewire.modals.admin-modal')
        @if (session('message'))
            <div class="alert alert-success text-black text-center" id="messagee">{{ session('message') }}</div>
        @endif
        <div class='btn btn-primary btn-sm float-end mb-2'  style="background-color: #638b33; color:white;" data-toggle="modal" data-target="#modal-permissions"> + Create New Permission</div>
